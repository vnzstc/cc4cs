...
sub printCsvReport {
	...
	$filename = "${prgname}Report.csv";
	open ($file, '>', $filename) or die "Open file failed";
	...
	$csvReport->printReportTableHeader($file);
	foreach my $incite (@incites) {
	...
	$csvReport->printReportTableFooter($file);
...
sub printTxtReport {
	...
	$txtReport->printReportTableHeader();
	foreach my $incite (@incites) {
	...
	$txtReport->printReportTableFooter();

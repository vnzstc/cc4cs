...
foreach my $incite (@incites) {
  ...
  # Init Parsing Variables.
	...
	$tsimFilePath = "$prjPath/tsim/$incite/${prgname}_TsimReport.txt";

  # Parsing gcov output file for total number of instruction executed.
	$cInstrExec = $parser->parseGCovFile($gcovFilePath);
  # Parsing isasim output file for Intel SPARC performance info.

	#($assemblyInstructions, $localExeTime, $localInstrPerSec,
	#	$clkCycles8051,$exeTime8051,$avg8051) =
	#	   $parser->parseTsimFile($tsimFilePath);

	($assemblyInstructions, $localExeTime, $clkCyclesSPARC, $exeTimeSPARC) =
		   $parser->parseTsimFile($tsimFilePath);
  ...
  #$clkCyclesAss = $assemblyInstructions * $avgCCHperAssemblyIntruction;
	...
	#$exeTimeSPARCus = $exeTimeSPARC * 1000 ;
	$exeTimeSPARCus = (1000000 / $freqSPARC) * $clkCyclesSPARC;

	$exeTimeSPARC = $exeTimeSPARCus / 1000;

	# SPARC frequency in Mhz
	#$freqSPARCMHz = $freqSPARC / 1000000 ;

	# average time took by SPARC microprocessor to execute 1 generic C intruction
	$timePerInstrSPARC = $exeTimeSPARCus / $cInstrExec ;
	# average number of clock cycles took by SPARC microprocessor
	# to execute 1 C intruction, this alue is computed using $timePerInstrSPARC
	$clockCyclePerInstrTpi = $timePerInstrSPARC * $freqSPARCMHz ;
	# average number of clock cycles took by SPARC microprocessor
	# to execute 1 C intruction, this alue is computed using $clkCyclesSPARC
	$clockCyclePerInstrClk = $clkCyclesSPARC / $cInstrExec ;

	# fill the incite array with computed values
	$statistics{$incite} = {
		cInstrExec              => $cInstrExec,
		assInstrExec            => $assemblyInstructions,
		execTimeSPARC           => $exeTimeSPARC,
		clockCycleAss           => $clkCyclesAss,
		clockCycleSPARC         => $clkCyclesSPARC,
		timePerInstrSPARC       => $timePerInstrSPARC,
...
sub printCsvReport {
	...
	$csvfilename = "${prgname}_sparcReport.csv";
	...
	foreach my $incite (@incites) {
		$csvReport->printReportTableEntry(
		  $csvfile,
			...
			$statistics{$incite}{execTimeSPARC},            #$execTimeSPARC,
			$statistics{$incite}{clockCycleAss},           #$clkCycleSPARC,
			$statistics{$incite}{clockCycleSPARC},          #$clkCycleSPARC,
			$statistics{$incite}{timePerInstrSPARC},        #$timePerInstrSPARC,
...
sub printTxtReport {
	...
	$txtfilename = "${prgname}_sparcReport.txt";
